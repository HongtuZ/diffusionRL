...
@partial(jax.jit,
         static_argnames=('action_decoder', 'num_samples', 'act_dim', 'eta', 'tau', 'need_ema'))
def jit_update_actor_with_dac(actor: Model,
                     actor_tar: Model,
                     critic: Model,
                     action_decoder: Callable,
                     batch: Batch,
    ...
    # dac q gradient
    Q_norm = jnp.abs(critic(batch.observations, batch.actions).reshape(2, batch_size)).mean()
    Q_grad_fn = jax.vmap(jax.grad(lambda a, s: critic(s, a).mean()))
    Q_grad = Q_grad_fn(noisy_actions, batch.observations) / (Q_norm + EPS)  # (B, dimA)

    rng, tr_key1 = jax.random.split(rng, 2)
    ...
    def actor_loss_fn(actor_paras: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        # dac q loss
        q_loss = (alpha_2 * Q_grad * pred_eps).mean()
...
class DQLDACLearner(Agent):
    ...
    def update(self, batch: Batch, use_dac=False) -> InfoDict:
        ...
        if use_dac:
            self.rng, self.actor, self.actor_tar, new_info = jit_update_actor_with_dac(self.actor,
                                                                                      self.actor_tar,
                                                                                      self.critic,
                                                                                      self.action_decoder,
                                                                                      batch,
